...
void BlurFilter::SetGaussianWeights(float sigma)
{
	...
	mBlurEffect.SetWeights(weights);
}
...
void BlurFilter::Init(ID3D11Device* device, UINT width, UINT height, DXGI_FORMAT format)
{
	...
	mBlurEffect.LoadEffect(L"FX/Blur.fx", device);
}
							 ...
							 int blurCount)
{
	...
	for(int i = 0; i < blurCount; ++i)
	{
		...
		mBlurEffect.HorzBlurTech->GetDesc(&techDesc);
		for(UINT p = 0; p < techDesc.Passes; ++p)
		{
			...
			mBlurEffect.SetInputMap(inputSRV);
			mBlurEffect.SetOutputMap(mBlurredOutputTexUAV);
			mBlurEffect.HorzBlurTech->GetPassByIndex(p)->Apply(0, dc);
		...
		mBlurEffect.VertBlurTech->GetDesc(&techDesc);
		for (UINT p = 0; p < techDesc.Passes; ++p)
		{
			...
			mBlurEffect.SetInputMap(mBlurredOutputTexSRV);
			mBlurEffect.SetOutputMap(inputUAV);
			mBlurEffect.VertBlurTech->GetPassByIndex(p)->Apply(0, dc);
