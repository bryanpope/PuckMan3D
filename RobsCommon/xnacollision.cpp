...
float OverlapSphereOrientedBox( const Sphere* pVolumeA, const OrientedBox* pVolumeB, XMFLOAT3& dir )
{
	XMASSERT( pVolumeA );
    XMASSERT( pVolumeB );
    ...
    // Choose value for each dimension based on the comparison.
    d = XMVectorSelect( d, -BoxExtents, LessThanMin );
    d = XMVectorSelect( d, BoxExtents, GreaterThanMax );
	...
	d = SphereCenter - d;
    // Use a dot-product to square them and sum them together.
    XMVECTOR d2 = XMVector3Length(d);

	XMVECTOR correct = d;
	correct = XMVector3Normalize(correct);
	correct = XMVector3Rotate(correct, BoxOrientation);
	XMStoreFloat3(&dir, correct);
	

	float overlap = pVolumeA->Radius - d2.m128_f32[0];
