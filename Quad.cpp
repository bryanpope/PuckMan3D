...
Quad::Quad(ID3D11Device* device, float width, float height)
{
	InitBuffers(device, width, height);
}

void Quad::InitBuffers(ID3D11Device* device, float width, float height)
{
	InitVB(device, width, height);
	InitIB(device);
}

void Quad::InitVB(ID3D11Device* device, float width, float height)
{
...
void Quad::InitIB(ID3D11Device* device)
{
	UINT indices[] = 
	{
		0, 2, 1,
